...
public class GenerateClassifierDataToRegAndBackAgain {
    ...
    public void launch() throws Exception {
        ...
        for (int i = 0; i < L; ++i) {
            ...
            cells[N].setCellType(CellType.DARK_TAPE);
            randomizeRegister(random, cells, N);
            ...
            for (int j = 0; j <= i - 1; ++j) {
                swap(cells, j + N);
                printFrameHeader(frame++, FINAL_FRAME, j == L - 2 && i == L - 1 ? 3 : 0.25);
                printRects(cells);
                printText(cells);
                printBorder(cells);
                printFrameFooter();                 
    ...
    private void randomizeRegister(final Random random, final Cell[] cells, final int N) {
        for (int i = N - 1; i >= 0; --i) {
            cells[i].setValue(random.nextInt(256));
        }
    }
    ...
    public static void main(final String... args) throws Exception {
        new GenerateClassifierDataToRegAndBackAgain().launch();
    }
}