...
public class LexerException extends RuntimeException {
    ...
    public LexerException(final Token token, final String message) {
        this(token.getFilename(), token.getLineNumber(), token.getLineColumn(), message);
    }
    
    public LexerException(final String filename, final int lineNumber, final int lineColumn, final String message) {
        super(message);
