...
public class Lexer {
    ...
    public String translate(final Map<String, Component> components, final String filename, final String tetrominoScript, 
    ...
    public Component tokenize(final Map<String, Component> components, final File file) 
            throws IOException, LexerException {
        return tokenize(components, file.getName(), new FileInputStream(file));
    }

    public Component tokenize(final Map<String, Component> components, final String filename) 
            throws IOException, LexerException {
        return tokenize(components, filename, new FileInputStream(filename));
    }
    
    public Component tokenize(final Map<String, Component> components, final File file, final InputStream in) 
            throws IOException, LexerException {
        return tokenize(components, file.getName(), in);
    }
    
    public Component tokenize(final Map<String, Component> components, final String filename, final InputStream in) 
            throws IOException, LexerException {
        return createComponent(deriveComponentName(filename), tokenize(filename, in), components);
    }
    
    private List<Token> tokenize(final String filename, final InputStream in) throws IOException, LexerException {
    ...
    private void tokenizeRange(final List<Token> tokens, final String filename, final int lineNumber, 
            final int lineColumn, final String value, final int rangeIndex) throws LexerException {
        ...
        try {
            ...
            throw new LexerException(filename, lineNumber, lineColumn, "Invalid range: left value is not a number.");
        }
        ...
        try {
            ...
            throw new LexerException(filename, lineNumber, lineColumn, "Invalid range: right value is not a number.");
        }
    ...
    private Component createComponent(final String name, final List<Token> tokens, 
            final Map<String, Component> components) throws LexerException {
        ...
        outer: while (true) {
            ...
            if (operationToken.getType() == TokenType.END) {
                ...
                throw new LexerException(operationToken, "Operation expected.");
            }
    ...
    private void verifyUniqueTerminalNames(final Token token, final Component component, final Set<String> names, 
        ...
        for (final Terminal terminal : terminals) {
            if (names.contains(terminal.getName())) {
                throw new LexerException(token, component.getName() + " has duplicate terminal name: " 
                        + terminal.getName());
            }
        }
    }
    ...
    private int processInstruction(final List<Token> tokens, final List<Instruction> instructions, 
            ...
            final int index) throws LexerException {
        ...
        if (moves.isEmpty()) {
            throw new LexerException(operationToken, "Missing moves.");
        }
        if (tetromino == null && moves.size() != 2) {
            throw new LexerException(operationToken, "Invalid component coordinates.");
        }
    ...
    private int processTerminals(final List<Token> tokens, final TerminalType type, final List<Terminal> terminals, 
        ...
        if (nameToken.getType() != TokenType.STRING) {
            throw new LexerException(nameToken, "Missing terminal name.");
        }
        ...
        while (true) {
            ...
            if (yToken.getType() != TokenType.NUMBER) {
                throw new LexerException(yToken, "Expected Y number.");
            }
